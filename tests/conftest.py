...
class SSEClient:
    ...
    def verify_event_order(self) -> bool:
        ...
        # intent 必须是第一个
        if not order or order[0] != "intent":
            return False
        ...
        # step_start 和 step_end 应成对
        step_starts = [i for i, e in enumerate(order) if e == "step_start"]
        step_ends = [i for i, e in enumerate(order) if e == "step_end"]

        if len(step_starts) != len(step_ends):
            return False

        # step_end 必须在对应的 step_start 之后
        for i, start_idx in enumerate(step_starts):
            if i >= len(step_ends) or step_ends[i] < start_idx:
                return False
    ...
    def get_all_steps(self) -> List[Dict]:
        """获取所有 step_start 事件"""
        return self.get_events_by_type("step_start")
