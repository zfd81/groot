...
│  🤔 Thinking...                             │  ← 思考过程（折叠显示）
│                                             │
│  ⚡ 调用技能: get-weather                    │  ← 技能调用
│  🔧 调用工具: file_read                      │  ← 工具调用
│                                             │
├─────────────────────────────────────────────┤
│  ┌ 补全弹窗 ──────────────────────────┐      │  ← 补全提示（按需显示）
│  │ /model  /skills  /mcp  /help ...  │      │
│  └────────────────────────────────────┘      │
├─────────────────────────────────────────────┤
│  > 用户输入区域                              │  ← 输入区（多行）
│                                             │
├─────────────────────────────────────────────┤
│  Ctrl+C 退出  Enter 发送  Tab 补全  Esc 取消 │  ← 状态栏
└─────────────────────────────────────────────┘
```
...
| 区域 | 说明 |
|------|------|
| 消息区 | 显示用户消息、助手回答、思考过程、技能/工具调用状态 |
| 补全弹窗 | 输入 `/` 时自动弹出，列出可用命令和 Skills |
| 输入区 | 多行文本输入，支持 Shift+Enter / Alt+Enter 插入换行 |
| 状态栏 | 显示当前模型、会话 ID、对话轮次、快捷键提示 |
...
|------|------|------|
| `/help` | 无 | 显示帮助信息 |
| `/model` | `[名称]` | 切换模型。无参数弹出模型列表选择，带参数直接切换 |
| `/clear` | 无 | 清空对话，开始新会话 |
| `/skills` | 无 | 列出已安装的 Skills，点击可快速填入 |
| `/mcp` | 无 | 查看可用的 MCP 工具列表 |
| `/export` | 无 | 导出当前对话为 Markdown 文件（保存到 `~/.groot/exports/`） |
| `/exit` | 无 | 退出 Chat TUI |
...
|------|------|
| `Enter` | 发送消息 |
| `Shift+Enter` / `Alt+Enter` | 在输入框中插入换行 |
| `Tab` | 接受补全建议 |
| `↑` / `↓` | 在补全列表中导航 / 历史输入 |
| `PgUp` / `PgDn` | 消息区翻页 |
| `ESC` | 关闭补全弹窗 / 取消当前对话 / 清空输入 |
| `Ctrl+C` | 退出 Chat TUI |
