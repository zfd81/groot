...
5. **文件管理**：重命名、删除（文件与空的非一级目录）、下载；在受限白名单位置内新建文件/子目录、上传
...
- 行悬停浮现「⋯」按钮，点击弹出操作菜单：
  - 文件：重命名 / 下载 / 删除
  - 目录：重命名 / 删除 / 新建文件* / 新建子目录* / 上传*（一级目录的重命名与删除禁用）
  - 带 * 的项仅在白名单位置出现（见 1.4.4）
...
POST   /web/files/mkdir            新建目录 {path}
POST   /web/files/create           新建空文件 {path}
...
新建文件、新建子目录、上传三类操作仅允许在以下位置（后端强制校验，前端同步隐藏入口）：

| 位置 | 新建文件/子目录 | 上传 |
|---|---|---|
| `skills/<名称>/` 及其任意深度子目录 | 允许 | 允许 |
| `skills/`（顶层） | 拒绝 | 允许 |
| `mcp/` | 拒绝 | 允许 |
| `subagents/` 顶层及 `subagents/<名称>/` 任意深度 | 拒绝 | 允许 |
| 其他位置 | 拒绝 | 拒绝 |

编辑保存、重命名、删除、下载不受此白名单限制（只受只读与隐藏规则约束）。
