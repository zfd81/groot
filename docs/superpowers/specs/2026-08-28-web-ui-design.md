...
| 聊天页（主界面） | 流式对话（增量渲染）、thinking 内容折叠展示、工具调用与结果折叠面板、子 Agent 事件标注、Markdown 渲染与代码高亮、附件上传、模型切换、Agent 切换（Solo 模式）、停止生成、可收起的左侧会话列表（分页）、继续任意历史会话、会话统计条（轮次 / 总耗时 / 累计输入与输出 Token / 模型） |
| 设置弹窗 | 通用设置（界面语言：中文 / English；外观主题：浅色 / 深色 / 跟随系统）；能力总览只读列表：子 Agents / Skills / MCP 工具 / 可用模型；集群管理：列出集群中全部实例（IP:PORT 地址、角色、加入时间、心跳时间），leader 实例带标签标识，列表定时自动刷新 |
...
- **输入区**：底部圆角卡片式输入框，工具条内嵌附件"+"按钮、模型切换下拉、Agent 切换（Solo 模式）、发送/停止按钮；输入框下方为会话统计条（轮次、会话总耗时、累计输入/输出 Token、当前模型，累计值由各轮消息求和，模型名来自对话详情端点）。
- **设置弹窗**：点击侧栏底部"设置"弹出模态窗，左侧为分类导航（通用设置 / 模型 / Skills / MCP 工具 / 子 Agents / 集群管理），右侧为内容区；通用设置含界面语言选择（右侧下拉：中文 / English）、外观主题选择（一排三个大卡片按钮：浅色 / 深色 / 跟随系统，各带图标，选中态以主题色高亮）与运行环境信息（工作目录、数据库类型、日志目录三行只读展示，每行左侧为加粗标题与灰色说明、右侧为等宽字体的值，数据来自 `GET /health` 的 `environment` 检查项），其余分类为只读能力列表。集群管理面板（`GET /web/cluster`）以卡片列表展示每个实例：等宽字体的 IP:PORT 地址、角色标签（leader 为绿色高亮标签，follower 为描边标签）、加入时间与心跳时间（毫秒时间戳按本地时区格式化）；leader 排在首位，其余按注册 ID 升序；面板打开期间每 5 秒静默轮询刷新，另提供手动刷新按钮。
...
### 2.5 v5 变更：设置弹窗新增集群管理面板

- 新增：`GET /web/cluster` 端点（登录会话保护，与其余 `/web` 端点同组），响应 `{"members":[{reg_id,role,address,pid,heartbeat_at,created_at}]}`；`address` 为 IP:PORT，时间字段为毫秒时间戳；leader 排首位，其余按 reg_id 升序。数据读取 `cluster_members` 表（过期成员由 leader 心跳循环清理，查询侧不再过滤）。
- 新增：`internal/api/handler/cluster.go`（`ClusterHandler`）及配套单元测试；`api.NewServer` 签名增加 `repo.MemberRepo` 参数，`main.go` 透传 `repos.Member`。
- 新增：设置弹窗左侧导航增加「集群管理」分类，对应 `ClusterPanel.vue` 卡片列表面板（leader 绿色标签 / follower 描边标签、加入时间、心跳时间、5 秒静默轮询 + 手动刷新）；对应中英文 i18n 文案（`settings.menuCluster` 与 `cluster.*` 命名空间）。
- 不变：集群注册 / 心跳 / 选主机制（`internal/cluster/`）、`cluster_members` 表结构、其余端点与页面。
