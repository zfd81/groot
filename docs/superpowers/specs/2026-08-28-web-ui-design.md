...
- **左侧栏（可收起）**：顶部为 Logo 与"新会话"按钮及收起按钮；中部为会话历史列表（标题 + 相对时间，分页加载）；提供仪表盘入口；底部为"设置"按钮。
...
- **设置弹窗**：点击侧栏底部"设置"弹出模态窗，左侧为分类导航（通用设置 / 模型 / Skills / MCP 工具 / 子 Agents），右侧为内容区；通用设置含界面语言选择（右侧下拉：中文 / English）与外观主题选择（一排三个大卡片按钮：浅色 / 深色 / 跟随系统，各带图标，选中态以主题色高亮），其余分类为只读能力列表。
