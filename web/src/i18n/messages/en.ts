// English messages. Keys must match zh-cn.ts one-to-one.
export default {
  common: {
    settings: 'Settings',
    newChat: 'New chat',
    arguments: 'Arguments',
    result: 'Result',
    model: 'Model',
    edit: 'Edit',
    delete: 'Delete',
    cancel: 'Cancel',
    close: 'Close',
    save: 'Save',
  },
  login: {
    header: 'Sign in to Groot',
    username: 'Username',
    password: 'Password',
    submit: 'Sign in',
    needCredentials: 'Please enter username and password',
    tooManyAttempts: 'Too many failed attempts, please try again later',
    failed: 'Sign-in failed',
  },
  setup: {
    header: 'Create User',
    desc: 'Welcome to Groot. Create a login user to get started.',
    username: 'Username',
    password: 'Password',
    passwordHint: 'Password (at least 8 characters)',
    confirmPassword: 'Confirm password',
    submit: 'Create',
    success: 'User created, please sign in',
    failed: 'Failed to create user',
    needAllFields: 'Please fill in username, password and confirmation',
    passwordTooShort: 'Password must be at least 8 characters',
    passwordMismatch: 'Passwords do not match',
  },
  password: {
    currentUser: 'Current user',
    title: 'Change Password',
    notifyTitle: 'Change Password',
    desc: 'After changing, sessions signed in from other browsers will be signed out.',
    oldPassword: 'Current password',
    newPassword: 'New password',
    newPasswordHint: 'New password (at least 8 characters)',
    confirmPassword: 'Confirm new password',
    submit: 'Change password',
    success: 'Password changed; other sessions signed out',
    failed: 'Failed to change password',
    wrongOldPassword: 'Current password is incorrect',
    needAllFields: 'Please fill in current password, new password and confirmation',
    tooShort: 'New password must be at least 8 characters',
    mismatch: 'New passwords do not match',
  },
  chat: {
    inputPlaceholder: 'Type a message. Enter to send, Shift+Enter for newline',
    heroTitle: 'How can I help you?',
    addAttachment: 'Add attachment',
    removeAttachment: 'Remove attachment',
    stop: 'Stop',
    send: 'Send',
    stopped: '_[Generation stopped]_',
    thinking: 'Thinking',
    round: 'Round {n}',
    duration: 'Elapsed {v}',
    tokenInputShort: 'In {n} tok',
    tokenOutputShort: 'Out {n} tok',
    thinkingLive: 'Thinking',
    timeUsed: 'Took {v}',
    durationSec: '{s}s',
    durationMinSec: '{m}m {s}s',
    copy: 'Copy',
    copied: 'Copied',
    copyFailed: 'Copy failed',
  },
  sidebar: {
    expand: 'Expand sidebar',
    collapse: 'Collapse sidebar',
    newChat: 'New chat',
    settings: 'Settings',
    search: 'Search chats',
    empty: 'No chats yet',
    rounds: '{n} rounds',
    loadMore: 'Load more',
    justNow: 'just now',
    minutesAgo: '{n} min ago',
    hoursAgo: '{n} h ago',
    daysAgo: '{n} d ago',
  },
  search: {
    placeholder: 'Search…',
    clear: 'Clear',
    recent: 'Recent chats',
    noResults: 'No matching topics',
    failed: 'Search failed, please retry',
    matchInstruction: 'My question',
    matchResult: 'AI reply',
  },
  logs: {
    title: 'Session Logs',
    viewLogs: 'View logs',
    refresh: 'Refresh',
    all: 'All',
    empty: 'No logs',
    emptyLevel: 'No logs at this level',
    failed: 'Failed to load logs',
    retry: 'Retry',
    count: '{n} entries',
    truncated: 'Only the latest 1000 entries are shown',
  },
  settings: {
    title: 'Settings',
    menuGeneral: 'General',
    menuAccount: 'Change Password',
    menuModels: 'Models',
    menuApiKeys: 'API Keys',
    menuCluster: 'Cluster',
    menuAgents: 'Agents',
    menuSkills: 'Skills',
    menuTools: 'MCP tools',
    language: 'Language',
    appearance: 'Appearance',
    appearanceDesc: 'Choose light, dark, or follow system',
    light: 'Light',
    dark: 'Dark',
    system: 'System',
    default: 'Default',
    apiUrl: 'API URL',
    modelName: 'Model name',
    noModels: 'No models available',
    addModel: 'New model',
    editModel: 'Edit model',
    deleteModelConfirm: 'Delete model "{name}"? This cannot be undone.',
    setDefault: 'Set as default',
    apiKey: 'API key',
    apiKeyKeepHint: 'Leave empty to keep the current key',
    advancedParams: 'Advanced parameters',
    testConnection: 'Test connection',
    testOk: 'Connection OK',
    testFail: 'Connection failed',
    modelSaved: 'Model saved',
    modelDeleted: 'Model deleted',
    defaultChanged: 'Default model changed',
    enable: 'Enable',
    disable: 'Disable',
    disabledTag: 'Disabled',
    noSkills: 'This agent has no skills',
    noTools: 'This agent has no MCP tools',
    noAgents: 'No sub-agents',
    agentLabel: 'Agent',
    viewAgentDef: 'View definition',
    viewAgentTitle: 'View · {name}',
    agentDefFile: 'Definition ({file})',
    agentDefLoadFail: 'Failed to load agent definition',
    viewAgentSkills: 'View skills',
    agentSkillsTitle: 'Skills · {name}',
    viewAgentTools: 'View MCP tools',
    agentToolsTitle: 'MCP tools · {name}',
    workDir: 'Working Directory',
    workDirDesc: 'Groot home directory for config and runtime data',
    dbType: 'Database',
    dbTypeDesc: 'Storage type for sessions and memory data',
    logDir: 'Log Directory',
    logDirDesc: 'Output location of server log files',
  },
  apikeys: {
    title: 'API Keys',
    desc: 'API Keys authenticate programmatic access to the public API via request header. Deleting a key revokes it immediately.',
    create: 'Create',
    createTitle: 'Create API Key',
    name: 'Name',
    namePlaceholder: 'e.g. my-service',
    nameRequired: 'Name is required',
    permissions: 'Permissions',
    permissionsRequired: 'Select at least one permission',
    expiresIn: 'Expires in',
    expires_1d: '1 day',
    expires_7d: '7 days',
    expires_1mo: '1 month',
    expires_6mo: '6 months',
    expires_1y: '1 year',
    expires_10y: '10 years',
    createdAt: 'Created',
    expiresAt: 'Expires',
    status: 'Status',
    valid: 'Valid',
    expired: 'Expired',
    actions: 'Actions',
    view: 'View',
    copyKey: 'Copy key',
    keyLabel: 'API Key',
    validity: 'Validity',
    empty: 'No API keys yet',
    tokenTitle: 'API Key: {name}',
    tokenDesc: 'Keep it safe. You can view and copy it here anytime.',
    copy: 'Copy',
    copied: 'Copied to clipboard',
    created: 'Created',
    copyFailed: 'Copy failed, please select and copy manually',
    deleteTitle: 'Delete API Key',
    deleteConfirm: 'Delete "{name}"? The key stops working immediately.',
    deleted: 'Deleted',
  },
  cluster: {
    title: 'Cluster',
    desc: 'Instances registered in the cluster, refreshed every 5 seconds. Members whose heartbeat times out are removed by the leader.',
    refresh: 'Refresh',
    leader: 'Leader',
    follower: 'Follower',
    joinedAt: 'Joined',
    heartbeatAt: 'Last heartbeat',
    empty: 'No cluster instances',
  },
  tool: {
    error: 'Error',
    done: 'Done',
    running: 'Running',
    arguments: 'Arguments',
    result: 'Result',
  },
  transcript: {
    thinking: 'Thinking',
    callAgent: 'Call agent',
    callSkill: 'Call skill',
    tool: 'Tool',
    arguments: 'Arguments',
    result: 'Result',
  },
  error: {
    unauthorized: 'Not signed in or session expired',
    requestFailed: 'Request failed ({status})',
    sessionBusy: 'This session already has an active conversation',
    connectionLost: 'Connection lost',
    generic: 'An error occurred',
  },
  files: {
    title: 'Workspace',
    refresh: 'Refresh',
    fullscreen: 'Fullscreen',
    exitFullscreen: 'Exit fullscreen',
    collapse: 'Collapse panel',
    newSkill: 'New Skill',
    newMcp: 'New MCP',
    newAgent: 'New Agent',
    namePrompt: 'Enter a name',
    scaffoldAgent: 'Owner agent',
    mainAgent: 'groot (main agent)',
    create: 'Create',
    cancel: 'Cancel',
    nameInvalid: 'Only letters, digits, underscores and hyphens are allowed, and it must start with a letter',
    fileNameInvalid: 'Must start with a letter or digit and contain no path separators',
    rename: 'Rename',
    renamePrompt: 'Enter a new name',
    download: 'Download',
    del: 'Delete',
    uploadFile: 'Upload file',
    uploadDir: 'Upload folder',
    uploadDirProgress: 'Uploading {done}/{total}',
    uploadDirDone: 'Uploaded {total} files',
    uploadDirTooMany: 'The selected folder has {count} files, over the per-batch limit of {limit}',
    uploadDirEmpty: 'The selected folder has no uploadable files',
    uploadDirExists: 'A folder named {name} already exists at the target; delete it and retry',
    uploadDirFailed: 'Upload stopped at {path} ({done}/{total} succeeded): {reason}',
    deleteConfirm: 'Delete {name}? This cannot be undone.',
    back: 'Back',
    edit: 'Edit',
    preview: 'Preview',
    save: 'Save',
    saved: 'Saved',
    unsavedConfirm: 'Discard unsaved changes?',
    binary: 'Binary file, cannot preview',
    tooLarge: 'File too large to view online',
    loadFailed: 'Failed to load',
    opFailed: 'Operation failed',
    readonlyTag: 'read-only',
    sync: 'Sync config',
    syncTitle: 'Config sync',
    syncScopeAll: 'All config',
    syncScopeOne: 'Sync this item',
    syncChecking: 'Comparing…',
    syncInSync: 'Local and database are already in sync.',
    syncDisabled: 'Config sync requires MySQL/PostgreSQL mode.',
    syncColStatus: 'Status',
    syncColPath: 'Path',
    syncStatusA: 'Local only',
    syncStatusM: 'Modified',
    syncStatusD: 'Database only',
    syncRemoteDeleted: 'Deleted by someone else',
    syncNeedsRestart: 'Restart after pull',
    syncPush: 'Push',
    syncPull: 'Pull',
    syncPushDeleteHint: 'Pushing will delete {count} file(s) from the database. This cannot be undone. Continue?',
    syncPullDeleteHint: 'Pulling will delete {count} local file(s). This cannot be undone. Continue?',
    syncPushDone: 'Pushed to database',
    syncPullDone: 'Pulled from database',
    syncRestartHint: 'Some changes require a service restart to take effect.',
    syncFailed: 'Sync failed',
    syncFileCount: '{count} file(s) changed',
  },
}
